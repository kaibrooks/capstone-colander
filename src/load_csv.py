...
def projectsHandler(projectsFileData):
        ...
        for value in projectsFileData[columnName]:
            if isinstance(value, object):
                try:
                    ...
                    pt.printError("{0} in the {1} column is not an integer. Terminating Program.".format(value, columnName))
            if isinstance(value, float):
                if value.is_integer():
                    value = int(value)
            if isinstance(value, int):
                if (value < 1) or (value > pow(2, 64)):
                    pt.printError("Value {0} in the {1} column must be an integer greater than zero and less than 2^64. Terminating Program".format(value, columnName))
    ...
    if projectsFileData.projectID.duplicated().any():
        pt.printError("projectid {0} is a duplicate projectID in the csv file. Terminating Program.".format({projectsFileData[projectsFileData.projectID.duplicated()].projectID.iloc[0]}))
    ...
    for min,max,pid in zip(minTeamSize,maxTeamSize,projectIDs):
        if min > max:
            pt.printError("minTeamSize is greater than maxTeamSize for projectID {0}.".format(pid))
    ...
    if projectIDGap > 0:
        pt.printWarning("gap found in projectID sequence in the projects csv file.")
...
def settingsHandler(settingsFileData):
    ...
    for row in settingsRows:
        if row not in settingsFileData['name'].values:
            pt.printError("Required {0} row 'name' not found in the settings csv file. Terminating Program.".format(row))
    ...
    def int_checker_settings(value, rowName, minValue):
        ...
        if isinstance(value, int):
            if (value < minValue) or (value > pow(2, 64)):
                 pt.printError("Value {0} in the {1} column must be an integer greater than zero and less than 2^64. Terminating Program".format(value, rowName))
            return value
        ...
        def is_integer_settings(value, rowName, minValue):
            if value.is_integer():
                ...
                pt.printError("{0} in the {1} row is not an integer. Terminating Program.".format(value, rowName))
    ...
    try:
        ...
        maxRunTime = 60
        pt.printWarning("maxRunTime value in settings csv is not an integer. Running with default value of 60 minutes.")
    if maxRunTime < 0:
        maxRunTime = 60
        pt.printWarning("maxRunTime in the settings csv is not a value between 1 and 2^64. Running with default value of 60 minutes.")
    ...
    try:
        ...
        pt.printError("The lowGPAThreshold 'min' value is not a float. Terminating program.")
    if (lowGPAThreshold < 0) or (lowGPAThreshold > 4.00) or (pd.isna(lowGPAThreshold)):
        pt.printError("lowGPAThreshold 'min' setting requires a 0.00 - 4.00 value. Terminating program.")
